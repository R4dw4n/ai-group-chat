...
interface RocketChatMessage {
  msg: 'method' | 'result' | 'connected' | 'ping' | 'pong' | 'sub' | 'ready' | 'changed';
  method?: string;
  id?: string;
  params?: any[];
  collection?: string;
...
export class RocketChatService {
  ...
  constructor(serverUrl: string = "wss://45-159-248-44.nip.io/websocket", token: string) {
    this.url = serverUrl;
  ...
  private authenticate(): void {
    if (!this.authToken) {
      console.warn("No auth token available. Connecting without authentication.");
      return;
    }
